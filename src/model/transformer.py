...
class FMT(nn.Module):
    def __init__(self, config):
        ...
        self.d_model = config['d_model']  # default 32
        self.nhead = config['nhead']  # default 8
        self.layer_names = config['layer_names']  # [self, cross] * 4
...
class FMT_with_pathway(nn.Module):
    ...
    def forward(self, features):
        ...
        for nview_idx, feature_multi_stages in enumerate(features):
            ref_fea_t_list = self.FMT(feature_multi_stages["stage1"], feat="ref")
            tmplist.append(ref_fea_t_list)
        ...
        for cur_view_idx in range(len(features_clone)):
            for nview_idx in range(len(features_clone)):
                if cur_view_idx == nview_idx:
                    continue
                ref_fea_t_list = self.FMT(features)
                features["stage1"]
